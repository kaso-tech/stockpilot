...
export default function PointOfSale() {
  ...
  const available = useMemo(() => products.filter(product => {
    ...
    const isLowStock = stockAfterCartSelection(product.quantity, 0, product.minimumQuantity).isLowStock;
    return product.quantity > 0 && matchesSearch && (!lowStockOnly || isLowStock);
  }), [lowStockOnly, products, query]);
  ...
  return <div className={`mx-auto max-w-[1600px] space-y-5 ${bottomSpacing}`}>
    <header className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-[#007B8B]">Vente comptoir</p><h1 className="mt-2 text-2xl font-semibold text-slate-950 dark:text-white sm:text-3xl">Point de vente</h1></div>
      {cart.length > 0 && <button type="button" onClick={() => setCartCollapsed(false)} className="inline-flex items-center gap-2 self-start rounded-xl border border-[#007B8B]/20 bg-[#007B8B]/[0.07] px-3 py-2 text-sm font-medium text-[#007B8B]"><ShoppingCart className="h-4 w-4" />{cart.length} article{cart.length > 1 ? "s" : ""} au panier{cartCollapsed && " · Ouvrir"}</button>}
    </header>
    <section className="rounded-2xl border border-border bg-card p-5">
      <div className="flex flex-col gap-2 sm:max-w-2xl sm:flex-row">
        <div className="relative flex-1"><Search className="pointer-events-none absolute left-3 top-3 h-4 w-4 text-muted-foreground" /><Input value={query} onChange={event => setQuery(event.target.value)} placeholder="Rechercher un produit ou une référence" className="pl-9" /></div>
        <div className="flex gap-2"><Button aria-label="Scanner un code-barres" variant="outline" size="icon" onClick={() => setCameraOpen(true)}><Camera className="h-4 w-4" /></Button><Button type="button" variant="outline" aria-pressed={lowStockOnly} onClick={() => setLowStockOnly(value => !value)} className={lowStockOnly ? "border-amber-500 bg-amber-500/10 text-amber-800 hover:bg-amber-500/15 dark:text-amber-200" : ""}><AlertTriangle className="mr-2 h-4 w-4" />Stock faible</Button></div>
      </div>
      <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {available.map(product => {
          const isLowStock = stockAfterCartSelection(product.quantity, 0, product.minimumQuantity).isLowStock;
          return <button key={product.id} onClick={() => add(product.id)} className={`rounded-xl border p-4 text-left transition hover:border-[#007B8B]/40 hover:bg-[#007B8B]/5 ${isLowStock ? "border-amber-500/50 bg-amber-500/[0.04]" : "border-border"}`}><p className="truncate font-medium">{product.name}</p><p className="mt-1 text-xs text-muted-foreground">{product.reference} · {product.quantity} {product.unit}</p>{isLowStock && <p className="mt-2 inline-flex items-center gap-1 rounded-md bg-amber-500/10 px-1.5 py-0.5 text-[11px] font-medium text-amber-700 dark:text-amber-300"><AlertTriangle className="h-3 w-3" />Stock faible · seuil {product.minimumQuantity}</p>}<div className="mt-4 flex items-center justify-between"><span className="font-semibold text-[#007B8B]">{formatCurrency(product.retailPriceCents)}</span><Plus className="h-4 w-4" /></div></button>;
        })}
      </div>
