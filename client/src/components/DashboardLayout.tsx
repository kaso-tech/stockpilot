  ...
  return (
    <>
      ...
      <SidebarInset className="min-w-0 bg-[#090c13] text-slate-100">
        <header className="app-header sticky top-0 z-40 flex h-16 items-center gap-3 border-b border-white/[0.07] bg-[#0e141f]/95 px-4 text-slate-100 shadow-lg shadow-black/20 backdrop-blur sm:px-6"><div className="lg:hidden"><SidebarTrigger className="h-9 w-9 rounded-lg border border-white/10 bg-[#111722] text-slate-200 hover:bg-white/[0.07]" /></div><div className="flex min-w-0 items-center gap-2"><activeMenuItem.icon className="h-4 w-4 shrink-0 text-primary" /><span className="truncate text-sm font-semibold text-slate-100">{activeMenuItem.label}</span></div><div className="ml-auto"><HeaderActions canRunBackup={canRunBackup} compact={isMobile} pending={runBackup.isPending} onBackup={() => runBackup.mutate()} notifications={notifications} onNotification={setLocation} isOnline={isOnline} pendingOfflineSales={pendingCount} hasFailedOfflineSale={offlineSales.some(sale => sale.status === "failed")} onSync={() => void syncNow()} /></div></header>
