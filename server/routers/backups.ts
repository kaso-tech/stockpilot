...
export const backupRouter = router({
  ...
  runNow: adminProcedure.mutation(async ({ ctx }) => { const settings = await getOrCreateSettings(); const archive = await runBackupWithStatus(ctx.user.id, "manual", settings.retentionCount); const db = await dbOrThrow(); await db.insert(auditLogs).values({ actorUserId: ctx.user.id, action: "Sauvegarde créée", entityType: "Sauvegarde", entityId: String(archive.id), detail: `Archive locale ${archive.filename} créée` }); return archive; }),
  download: adminProcedure.input(z.object({ id: z.number().int().positive() })).mutation(async ({ input }) => ({ url: await getBackupDownloadUrl(input.id) })),
