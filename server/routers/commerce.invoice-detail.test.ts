...
import { agents, customers, saleCommissions, saleItems, sales, users } from "../../drizzle/schema";
...
describe("commerce.sales.detail", () => {
  beforeEach(() => {
    ...
    const db: any = { select: () => ({ from: (table: unknown) => { const rows = table === sales ? [sale] : table === customers ? [{ id: 4, name: "Client pro", type: "wholesale" }] : table === saleItems ? [{ id: 1, productName: "Article", quantity: 2, unitPriceCents: 12000, lineTotalCents: 24000 }] : table === saleCommissions ? [] : table === users ? [{ id: 3, name: "Kadré Vendeur", email: null }] : table === agents ? [agentRead++ === 0 ? { id: 7, name: "Aminata Commerciale" } : { id: 8, name: "Moussa Caissier" }] : []; const chain = Object.assign(rows, { limit: async () => rows }); return { where: () => chain, then: (resolve: any) => resolve(rows) }; } }) };
