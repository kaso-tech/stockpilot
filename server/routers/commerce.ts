...
export const commerceRouter = router({
  ...
  settings: router({
    ...
    save: adminProcedure.input(z.object({ defaultSalesAgentId: z.number().int().positive().nullable(), defaultCashierId: z.number().int().positive().nullable(), requireSalesAgent: z.boolean(), requireCashier: z.boolean() })).mutation(async ({ ctx, input }) => { const db = await dbOrThrow(); const current = (await db.select().from(saleSettings).limit(1))[0]; const data = { ...input, updatedByUserId: ctx.user.id }; if (current) await db.update(saleSettings).set(data).where(eq(saleSettings.id, current.id)); else await db.insert(saleSettings).values(data); await audit(ctx.user.id, "Paramètres mis à jour", "Vente", null, "Affectation des agents mise à jour"); return { success: true }; }),
  }),
  remuneration: router({
