...
export const commerceRouter = router({
  ...
  sales: router({
    ...
    detail: protectedProcedure.input(z.object({ id: z.number().int().positive() })).query(async ({ input }) => { const db = await dbOrThrow(); const sale = (await db.select().from(sales).where(eq(sales.id, input.id)).limit(1))[0]; if (!sale) throw new TRPCError({ code: "NOT_FOUND", message: "Facture introuvable." }); const [customer, items, commissions, seller, salesAgent, cashier] = await Promise.all([(await db.select().from(customers).where(eq(customers.id, sale.customerId)).limit(1))[0], db.select().from(saleItems).where(eq(saleItems.saleId, sale.id)), db.select().from(saleCommissions).where(eq(saleCommissions.saleId, sale.id)), (await db.select().from(users).where(eq(users.id, sale.sellerUserId)).limit(1))[0] ?? null, sale.salesAgentId ? (await db.select().from(agents).where(eq(agents.id, sale.salesAgentId)).limit(1))[0] ?? null : null, sale.cashierId ? (await db.select().from(agents).where(eq(agents.id, sale.cashierId)).limit(1))[0] ?? null : null]); return { sale, customer, items, commissions, participants: { seller: seller ? { id: seller.id, name: seller.name || seller.email || "Vendeur", role: "Vendeur" } : null, salesAgent: salesAgent ? { id: salesAgent.id, name: salesAgent.name, role: "Agent commercial" } : null, cashier: cashier ? { id: cashier.id, name: cashier.name, role: "Caissier" } : null } }; }),
