...
const signature = "data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHdpZHRoPSI4MCIgaGVpZ2h0PSI0MCI+PHRleHQgeD0iMCIgeT0iMzAiPk9LPC90ZXh0Pjwvc3ZnPg==";
const identity = { id: 1, defaultSalesAgentId: null, defaultCashierId: null, requireSalesAgent: false, requireCashier: false, currency: "XOF", companyName: "StockPilot", companyLogoUrl: null, companySignatureUrl: null, companySignatureLabel: "Signature & cachet", companyAddress: null, companyPhone: null, companyEmail: null, companyFooter: null };
...
await page.route("**/api/trpc/**", async route => {
  ...
  const value = path => path === "auth.me" ? admin : path === "commerce.settings.get" ? identity : path === "commerce.sales.list" ? [sale] : path === "commerce.sales.detail" ? detail : path === "commerce.settings.uploadLogo" ? { url: logo } : path === "commerce.settings.uploadSignature" ? { url: signature } : { success: true };
  if (paths.includes("commerce.settings.uploadLogo")) identity.companyLogoUrl = logo;
  if (paths.includes("commerce.settings.uploadSignature")) identity.companySignatureUrl = signature;
  if (paths.includes("commerce.settings.save")) { identitySaved = true; Object.assign(identity, { companyName: "Bati Pro", companySignatureLabel: "Cachet autorisé", companyAddress: "Ouagadougou, Koulouba", companyPhone: "+226 70 00 00 00", companyEmail: "contact@batipro.test", companyFooter: "NIF : BF-TEST" }); }
...
await page.waitForFunction(() => document.body.textContent?.includes("Signature ou cachet enregistré."));
...
await page.getByLabel("Libellé signature/cachet").fill("Cachet autorisé");
...
await page.locator("#invoice-print").getByText("Cachet autorisé", { exact: true }).waitFor();
if (await page.locator('#invoice-print img[alt="Signature ou cachet Bati Pro"]').count() !== 1) throw new Error("Le cachet configuré n’est pas affiché sur la facture A4.");
await browser.close();
